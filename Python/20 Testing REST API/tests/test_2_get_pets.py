...
def test_3_get_pets_expired_key(auth_key=expired_api_key, filter='my_pets'):
...
def test_4_get_pets_auth_key_number(auth_key=1354, filter='my_pets'):
    assert auth_key != valid_key
    headers = {'auth_key': auth_key}
    params = {'filter': filter} if filter != None else None
    t = datetime.now()
...
def test_5_get_pets_wrong_key(auth_key='abc', filter='my_pets'):
    headers = {'auth_key': auth_key}
    params = {'filter': filter} if filter != None else None
    t = datetime.now()
    status, headers, resp = request('get', 'api/pets', headers=headers, params=params)
...
def test_6_get_pets_wrong_filter(auth_key=valid_key, filter='pets'):
    assert 50 < len(auth_key) < 65
    headers = {'auth_key': auth_key}
    params = {'filter': filter} if filter != None else None
    t = datetime.now()
...
def test_7_get_pets_wrong_method(method='patch', auth_key=valid_key, filter='my_pets'):
    headers = {'auth_key': auth_key}
    params = {'filter': filter} if filter != None else None
    t = datetime.now()
...
def test_8_get_pets_wrong_ct_type(auth_key=valid_key, filter='pets', content_type='application/xml; '
...
def test_9_get_pets_filter_255(auth_key=valid_key, filter=rnd_str(255)):
    assert 50 < len(auth_key) < 65
    headers = {'auth_key': auth_key}
    params = {'filter': filter} if filter != None else None
    t = datetime.now()
...
def test_10_get_pets_filter_1025(auth_key=valid_key, filter=rnd_str(1025)):
    assert 50 < len(auth_key) < 65
    headers = {'auth_key': auth_key}
    params = {'filter': filter} if filter != None else None
    t = datetime.now()
...
def test_11_get_pets_filter_spec_symbols(auth_key=valid_key, filter=rnd_str(symbols='s')):
    assert 50 < len(auth_key) < 65
    headers = {'auth_key': auth_key}
    params = {'filter': filter} if filter != None else None
    t = datetime.now()
...
def test_12_get_pets_non_ascii_filter(auth_key=valid_key, filter=rnd_str(symbols='ru')):
    assert 50 < len(auth_key) < 65
    headers = {'auth_key': auth_key}
    params = {'filter': filter} if filter != None else None
    t = datetime.now()
